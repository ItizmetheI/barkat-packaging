...
import textureLoop from '../assets/video/texture-loop.mp4'
...
export default function ProcessGallery() {
  ...
  // Same imperative-muted fix as HeroVideo, plus IntersectionObserver-gated: this section
  // is below the fold, so starting the 6MB texture loop on mount (before the visitor has
  // scrolled anywhere near it) would eagerly download it for nothing.
  useEffect(() => {
  ...
  return (
    <section style={{ position: 'relative', padding: '96px 6%', overflow: 'hidden', background: '#0A0E14' }}>
      <video
        ref={videoRef}
        muted
        loop
      ...
      >
        <source src={textureLoop} type="video/mp4" />
      </video>
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(10,14,20,0.85)' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 32 }}>
          {STEPS.map((step, i) => (
            <Reveal key={step.eyebrow} delay={(i % 3) * 0.1}>
              <div style={{ background: '#0F1520', border: '1px solid rgba(247,248,250,0.08)' }}>
